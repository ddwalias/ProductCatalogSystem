<script lang="ts">
  ...
  const activeCategoryOptions = $derived(
    categoryOptions.filter((category) => category.status === "Active"),
  );
  const activeCategoryCount = $derived(activeCategoryOptions.length);
  const inactiveCategoryCount = $derived(
    categoryOptions.length - activeCategoryCount,
  );
  ...
  const selectedProductAttributes = $derived(
    getAttributeEntries(selectedProduct?.customAttributes ?? null),
  );
  ...
  const querySummary = $derived(
    appliedSearch
      ? `Querying for "${appliedSearch}"`
      : "Showing the full catalog canvas.",
  );
  ...
  async function refreshCategories() {
    ...
    try {
      ...
      const nextActiveOptions = nextOptions.filter(
      ...
      if (
        !focusedCategoryId ||
        !findCategoryById(nextCategories, focusedCategoryId)
      ) {
        focusedCategoryId = nextCategories[0]?.id ?? null;
      }

      if (categoryForm.id) {
        const refreshedCategory = findCategoryById(
          nextCategories,
          categoryForm.id,
        );
  ...
  async function removeSelectedProduct() {
    ...
    const confirmed = window.confirm(
      `Delete "${productForm.name}" from the catalog?`,
    );
    if (!confirmed) {
      return;
    }
  ...
  async function submitCategoryForm() {
    ...
    try {
      ...
      categoryForm = refreshed
        ? categoryToForm(refreshed)
        : categoryToForm(saved);
    } catch (error) {
  ...
  function buildCategoryPayload(): CategoryWritePayload {
    return {
      ...
      parentCategoryId: categoryForm.parentCategoryId
        ? Number(categoryForm.parentCategoryId)
        : null,
...
<div class="page-shell">
  ...
  <header class="masthead">
    <div>
      ...
      <p class="lede">
        Search with autocomplete, curate products with revision-safe editors,
        shape category hierarchies, and inspect live category slices without
    ...
    <div class="masthead-actions">
      <div
        class:checking={health.tone === "checking"}
        class:healthy={health.tone === "healthy"}
        class:degraded={health.tone === "degraded"}
        class="health-chip"
      >
      ...
      <div class="quick-links">
        <button
          class="ghost-button"
          type="button"
          onclick={() => void refreshHealth()}>Refresh health</button
        >
        <a
          class="ghost-link"
          href="/api/health"
          rel="noreferrer"
          target="_blank">Open health</a
        >
        <a class="ghost-link" href="/swagger" rel="noreferrer" target="_blank"
          >Open Swagger</a
        >
      </div>
    </div>
  ...
  <section class="hero-panel panel">
    <div class="hero-copy">
      ...
      <p>
        The command search is backed by the dedicated autocomplete endpoint,
        while the larger stream below stays tied to the paged product list
        endpoint.
      </p>
    ...
    <div class="hero-metrics">
      <article>
        ...
        <p>
          Paged from `GET /api/products` with filters, sort, and pagination.
        </p>
      </article>
      <article>
        ...
        <p>
          {inactiveCategoryCount} inactive categories remain visible for maintenance.
        </p>
      </article>
      <article>
        <span
          >{focusedCategory
            ? formatNumber(categoryProducts.totalCount)
            : "Pick one"}</span
        >
  ...
  <main class="workspace">
      ...
      <section class="panel control-panel">
        ...
        <div class="filter-footer">
          <div class="active-filters">
            ...
            {#if priceFromFilter}
              <span class="filter-chip"
                >From {formatMoney(Number(priceFromFilter))}</span
              >
            {/if}
            {#if priceToFilter}
              <span class="filter-chip"
                >To {formatMoney(Number(priceToFilter))}</span
              >
            {/if}
          </div>
          ...
          <button
            class="ghost-button"
            type="button"
            onclick={resetCatalogFilters}>Reset</button
          >
        </div>
        ...
        <div class="product-grid">
          {#if products.items.length === 0 && !productsBusy}
            <div class="empty-state">
              ...
              <p>
                Relax the filters, clear the query, or start a new product from
                the studio.
              </p>
            </div>
          {/if}
            ...
            >
              ...
              <div class="product-card-copy">
                ...
                <div class="inventory-track">
                  <span
                    style={`width:${inventoryMeter(product.inventoryOnHand)}`}
                  ></span>
                </div>
                ...
                <div class="product-card-footer">
                  ...
                  <small
                    >{product.inventoryOnHand} on hand · Updated {formatDate(
      ...
      <section class="panel category-pulse-panel">
        <div class="section-heading">
          <div>
            ...
            <h2>
              {focusedCategory ? focusedCategory.name : "Pick a category"}
            </h2>
          </div>
          {#if focusedCategory}
            <button
              class="ghost-button"
              type="button"
              onclick={() => beginCreateCategory(focusedCategory.id)}
              >Create child</button
            >
          {/if}
        </div>
        ...
        <div class="category-product-grid">
          {#if !focusedCategory}
            <div class="empty-state">
              ...
              <p>
                Use the category atlas to inspect the dedicated `GET
                /api/categories/{"{id}"}/products` endpoint.
              </p>
            </div>
          {:else if categoryProducts.items.length === 0 && !categoryProductsBusy}
            <div class="empty-state">
              ...
              <p>
                This category is ready for new inventory or a tighter parent
                grouping.
              </p>
            </div>
          {:else}
            {#each categoryProducts.items as product (product.id)}
              ...
              >
                <div>
                  ...
                  <small
                    >{formatMoney(product.price)} · Qty {product.inventoryOnHand}</small
                  >
                </div>
                <span>Open</span>
              </button>
    ...
    <div class="studio-stage">
      <section class="panel spotlight-panel">
        <div class="section-heading">
          ...
          {#if selectedProduct}
            <button
              class="ghost-button"
              type="button"
              onclick={() => void loadProduct(selectedProduct!.id)}
              >Reload detail</button
            >
          {/if}
        </div>
        ...
        {#if selectedProduct}
          ...
          <p class="spotlight-description">
            {selectedProduct.description ??
              "No product description provided yet."}
          </p>
          ...
          <div class="attribute-panel">
            ...
            {#if selectedProductAttributes.length === 0}
              <p class="muted">
                This product does not currently expose custom attributes.
              </p>
            {:else}
        ...
        {:else}
          <div class="empty-state">
            ...
            <p>
              Pick a result card, use autocomplete, or load a category product
              to inspect the dedicated detail endpoint.
            </p>
          </div>
        {/if}
      </section>
      ...
      <section class="panel editor-panel">
        <div class="section-heading">
          ...
          <div class="editor-actions">
            <button
              class="ghost-button"
              type="button"
              onclick={beginCreateProduct}>Reset draft</button
            >
            <button
              class="danger-button"
              type="button"
        ...
        {#if productMessage}
          <p
            class="status {Object.keys(productErrors).length > 0
              ? 'error'
              : 'success'}"
          >
            {productMessage}
        ...
        >
          ...
          <label>
            ...
            <textarea
              ...
              placeholder="Describe the product, audience, and product feel."
              rows="4"
            ></textarea>
          </label>
              ...
              {#each attributeRows as row (row.id)}
                <div class="attribute-row">
                  ...
                  <button
                    class="ghost-button"
                    type="button"
                    onclick={() => removeAttributeRow(row.id)}>Remove</button
                  >
                </div>
              {/each}
            </div>
          ...
          <div class="form-grid">
            <label>
              ...
              <input
                bind:value={productForm.inventoryReason}
                placeholder="Cycle count, seasonal restock…"
              />
            ...
            <label>
              ...
              <input
                bind:value={productForm.changedBy}
                placeholder="catalog-admin"
              />
      ...
      <section class="panel category-panel">
        <div class="section-heading">
          ...
          <button
            class="ghost-button"
            type="button"
            onclick={() => beginCreateCategory()}>New root category</button
          >
        </div>

        {#if categoryMessage}
          <p
            class="status {Object.keys(categoryErrors).length > 0
              ? 'error'
              : 'success'}"
          >
            {categoryMessage}
          ...
          >
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="Define what belongs here and what should stay out."
                rows="4"
              ></textarea>
            </label>
